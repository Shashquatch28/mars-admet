...
**Currently applied:** DILI augmented with DILIst (FDA, same lineage/superset as TDC's source — adopted directly). BBB and Clearance's candidate augmentation source (PharmaBench) was **dropped 2026-08-21 following the licensing check in §7** — remain TDC-only for now.

### 7. Dataset licensing check
`LAUNCH SCOPE: MVP` — added 2026-08-21, resolving the review's flagged gap ("dataset licensing isn't verified anywhere... conspicuous given how carefully everything else is gated").

**All 14 TDC-sourced endpoint datasets (Module 2) confirmed CC BY 4.0**, verified directly against TDC's own per-dataset license pages — this includes all three AstraZeneca-released sets the review specifically named (Lipophilicity_AstraZeneca, PPBR_AZ, Clearance_Microsome_AZ). Several show "Not Specified. CC BY 4.0" — meaning the *original* source (e.g. AstraZeneca's 2016 disclosure) didn't itself declare a license, but TDC's own redistribution terms are CC BY 4.0 regardless, which is what governs downstream use. Attribution-only, commercial use permitted.
...
**PharmaBench** (candidate augmentation source for BBB/Clearance, previously "pending concordance check") is licensed **CC BY-NC-ND 4.0** per its official Nature *Scientific Data* publication — confirmed directly from the paper's Rights and Permissions section, not inferred. This is a real constraint, not a formality:
...
`LAUNCH SCOPE: MVP` — resolved 2026-08-21, previously unaddressed (flagged in review: default RDKit/ECFP settings can silently include or strip stereo depending on configuration).

**Evidence base:** RDKit's Morgan/ECFP fingerprint generation has an optional chirality flag (`useChirality`) that **defaults to off** — confirming the review's concern exactly; two molecules differing only in R/S configuration produce identical ECFP4 fingerprints unless this is explicitly enabled. This isn't a theoretical concern for MARS specifically: a quantitative meta-analysis (Nakamura et al., spanning 45 metabolic reactions across 19 substrates) found that while most CYP isoforms show only modest R/S enantiomer differences in metabolic rate (median Km/Vmax ratios 0.80–1.53 for CYP1A2/2B6/2C19/2D6/3A4), **CYP2C9 — one of MARS's three CYP endpoints — shows systematic, larger differences** (median Vmax and CLint ratios of 0.43 and 0.60). The textbook case is warfarin: S-warfarin is metabolized by CYP2C9, R-warfarin by CYP1A2/CYP3A4 — different enzymes entirely, not just different rates. Stereoselectivity is also documented at the transporter level (relevant to P-gp, BBB, Caco-2 permeability endpoints), so this isn't limited to the CYP cluster.
...
`LAUNCH SCOPE: MVP` — added 2026-08-21, given confirmed access to college lab A100s.
...
`LAUNCH SCOPE: Split` — §1-5 (metrics, CV/reporting protocol, significance testing, reproducibility-gated leaderboard comparison, self-audit protocol) are **MVP**: need to know models actually work before shipping predictions to users. §5.5 (supplementary prospective-performance robustness check, added 2026-08-21) and §6 (full ablation matrix — 14 endpoints × {baseline, single-task, multi-task} × 5 seeds × 6 ablation axes as of 2026-08-21's calibration addition) are both **Post-MVP-parallel**: valuable for the publication/credibility angle, doesn't block product launch. **Compute budget resolved 2026-08-21 (Module 10)** — actual cost is small (~$40 MVP, ~$75 total including the full ablation matrix), so this isn't the schedule risk it looked like on paper; it's still real wall-clock time across 100+ discrete runs to launch and monitor, but not a cost blocker. §7 (baseline comparison table format) follows whichever of the above it's reporting on.
...
**5.5. Supplementary prospective-performance robustness check** `LAUNCH SCOPE: Post-MVP-parallel` — added 2026-08-21, motivated by Module 4's scaffold-split limitation caveat.
